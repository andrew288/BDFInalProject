	...
	//propiedades
		...
		public void setDataReferencial() {
			...
			try {
				...
				rs = st.executeQuery("select * from Tipo_Rol");
				while(rs.next()) {
					Object[] aux= {rs.getInt("TipRolCod"),rs.getString("TipRolDes"),rs.getString("TipRolEstReg")};
					modelo.addRow(aux);
				}
				mysql.getConn().close();
			} catch (Exception e){
		...
		public void tipoServicioAdmin() {
			...
			subtitleA.setText("Registro Tipo Rol");
			...
			subtitleB.setText("Tabla_Tipo_Rol");
					...
					if(CarFlaAct!=0) {
						...
						if(CarFlaAdi==1) {
							if(input_des.getText().length()!=0 && input_cod.getText().length()!=0) {
								...
								try {
									st= mysql.getConn().prepareStatement("INSERT INTO Tipo_Rol VALUES(?,?,?)");
									st.setInt(1,cod);
									st.setString(2,des);
						...
						if(CarFlaMod==1) {
							...
							if(fila>=0) {
								...
								try {
									st= mysql.getConn().prepareStatement("UPDATE Tipo_Rol SET TipRolDes=? WHERE TipRolCod=?");
						...
						if(CarFlaEli==1) {
							...
							if(fila>=0) {
								int codFila=Integer.parseInt(input_cod.getText());
								try {
									st= mysql.getConn().prepareStatement("UPDATE Tipo_Rol SET TipRolEstReg=? WHERE TipRolCod=?");
									st.setString(1,"*");
									st.setInt(2, codFila);
									int retorno = st.executeUpdate();
									//aplicamos los cambios en el modelo
