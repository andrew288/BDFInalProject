	...
	//propiedades
		...
		public void setDataReferencial() {
			...
			try {
				...
				while(rs.next()) {
					Object[] aux= {rs.getInt("TipProCod"),rs.getString("TipProDes"),rs.getString("TipProEstReg")};
					modelo.addRow(aux);
				}
				mysql.getConn().close();
			} catch (Exception e){
		...
		public void tipoServicioAdmin() {
			...
			ventana.setTitle("Tipo Producto");
			...
			subtitleA.setText("Registro Tipo Producto");
			...
			subtitleB.setText("Tabla_Tipo_Producto");
					...
					if(CarFlaAct!=0) {
						...
						if(CarFlaAdi==1) {
							if(input_des.getText().length()!=0 && input_cod.getText().length()!=0) {
								...
								try {
									st= mysql.getConn().prepareStatement("INSERT INTO Tipo_Producto VALUES(?,?,?)");
									st.setInt(1,cod);
									st.setString(2,des);
						...
						if(CarFlaMod==1) {
							...
							if(fila>=0) {
								...
								try {
									st= mysql.getConn().prepareStatement("UPDATE Tipo_Producto SET TipProDes=? WHERE TipProCod=?");
						...
						if(CarFlaEli==1) {
							...
							if(fila>=0) {
								int codFila=Integer.parseInt(input_cod.getText());
								try {
									st= mysql.getConn().prepareStatement("UPDATE Tipo_Producto SET TipProEstReg=? WHERE TipProCod=?");
									st.setString(1,"*");
									st.setInt(2, codFila);
									int retorno = st.executeUpdate();
									//aplicamos los cambios en el modelo
